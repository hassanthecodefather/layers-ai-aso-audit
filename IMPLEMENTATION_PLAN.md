...
- `domain/identity.ts` (types: IdentityVersion, the signal-family tally, two-axis bands) + `mastra/tools/resolve-identity.ts`. The resolver **consumes the workflow's already-gathered listing** (no second iTunes fetch) — so the §G "modify identify-app.ts" note is satisfied this way; no separate change to that file is required.
- Deterministic day-one signals **and where they fire (honest sourcing, per spec):** developer + other-apps, bundle-id reverse-DNS, marketing-domain match (crawler **[live]**), review-vocabulary **fire today**. **IAP names + App-Privacy "nutrition" labels** are product-page only → recover via the crawler as **optional enrichment** (not in Lookup). **iOS permission usage strings are binary-only → not publicly observable**; model the family but it reports `not observed` (don't promise a signal that can't fire). Pure-code matching, no vision (that's ID-full at P2).
...
- Escalation **detection + safe fallback** here: on conflict (escalate band) the audit proceeds but **suppresses identity-rewriting recs and stamps "identity unconfirmed."** The **interactive** confirm/correct/pick prompt is its own step — **A5** (below) — so A2 ships the no-engagement branch first. Writes `aso_identity_versions` stage=`lite`.
...
**A5 · Interactive escalation + human-confirmed override [live]** — *the interactive half of escalation; A2 shipped only the safe fallback.*
- **Widen the existing `confirm-app` suspend step** (`audit-workflow.ts`): extend `suspendSchema` to carry the resolved identity + competitor set + candidate alternatives, and `resumeSchema` from `{ confirmed }` to `{ confirmed, correctedIdentity?, chosenCandidate? }`.
- **Only escalate to a real decision when the band is `escalate`.** For high-confidence apps the identity is shown informationally — so TikTok/Spotify still produce **zero asks** (don't regress §F).
- On resume with a correction/choice, write an `aso_identity_versions` row `source = 'human_confirmed'` (sticky, highest-priority tier). This also lights up the **human-confirmed override** path; "reuse, don't recompute" then reuses it next run.
- **TDD first (§F ID-lite, extended):** Rivian → escalate → confirm step surfaces identity options → resume with a correction → persists as `human_confirmed` → re-audit **reuses it (no re-ask)** and now **permits** identity-rewriting recs (confirmed); TikTok/Spotify still **zero asks**.
- **DoD:** §F ID-lite (incl. the interactive + human-confirmed assertions) **and** §F P1 green; second audit references the first, marks applied, never repeats. *Phase A is "done" only when A5 is green — the safe fallback alone is not the whole escalation.*
- **DoD — recommendation-set stability (the real Phase-A bar), scoped.** With **stateless generation** (no ledger in the prompt) and **temp ≈ 0**, two consecutive live audits of an unchanged listing produce a **stable set of deterministic-signal-derived recs** — run 2 adds ≈ 0 such rows and drops none. *(Score stability is its own bar — see **A6**. The earlier "scores may wobble ±1–2; fine" was wrong: live testing showed the overall swinging **46 → 30** at temp 0 because per-dimension scores are free model output. A6 makes the deterministic dimensions code-scored and reused; until then, treat the **rec set** as the Phase-A bar and the **score** as a known-unstable A6 item.)* *"No duplicate `rec_key`s" is the wrong gate — it passes even when the set is pure model noise.* **Keyword recs are explicitly out of scope for this bar** — their deterministic candidate engine is **P3**, so they stay model-driven (and may drift) until then; assert stability on the structural dimensions only, and re-assert full stability (incl. keywords) at the end of **P3**. The live drift we saw was two causes: the ledger-injection feedback loop (fixed by stateless generation — most of it) and residual temp-0 variance on structural dims (fixed by deterministic candidates); keyword fan-out is the P3 remainder.

**A6 · Score determinism — honour "deterministic signals solid, LLM scores muted" (spec line 173) [live].** *The Phase-A frontier surfaced by live testing; the §F P1 gates pass, this closes a P1 secondary-uplift gap.* **Status: ✅ complete and green (186 hermetic tests)** — code-derived confidence, both reuse layers, identity temp-0, code-scoring for screenshots / preview-video / ratings, the title/subtitle coarse-ordinal step, and the docstring cleanup are all done. Two **beta-calibration caveats** are tracked below for the 6b golden-set retune (spec §C/§E), not blockers.

Observed in two live Rivian re-runs on the *same* listing at temperature 0: overall **46 then 30** — a 16-pt swing. It breaks three P1 promises: "LLM scores muted" (line 173), the usefulness of zero-LLM rubric replay (line 170 — replaying a noisy draft replays noise), and the change-diff / score-delta panel (a delta only means something if untouched dimensions hold).

Root cause is **structural, not the temperature flag** — temp 0 *is* applied (`scoring/score.ts:53`). Variance enters because the code/LLM split stops at the *total* instead of reaching each *dimension*:
- The overall is a weighted mean of the model's **free 0-10 per-dimension scores** (`scoring/aggregate.ts:62`); nothing maps the deterministic signals to the *number*, so even fact-based dimensions (screenshot count, char-utilisation) drift ±1–3 and, weighted, sum to ~16.
- The **model-authored `confidence`** silently controls the normalization denominator (`aggregate.ts:54` counts only `confidence !== 'unavailable'`), so one dimension flipping `inferred`↔`unavailable` re-weights every other dimension.
- The **repair path** (`score.ts:80`) re-generates a fresh draft on a schema miss → a different sampling.

Fix — push the existing code/LLM split down from the total to each dimension:
1. ✅ **Code-derive assessability / `confidence`** — done (`scoring/dimension-scorer.ts: deriveConfidence`, applied in `aggregate.ts:61`). A dimension is `unavailable` iff its inputs are unobservable (the crawler-gated subtitle / preview / promo), decided in code, never by the model. The floating-denominator amplifier is gone.
2. ✅ **Code-score the deterministic dimensions** from the signals fact sheet (`scoring/dimension-scorer.ts: codeScore`, applied in `aggregate.ts`): **screenshots** = slots-used-of-10, **previewVideo** = present→8 / absent→0, **ratings** = `(allTimeAverage / 5)·10` with a ±1 recent-trend nudge (rubric checks 1–2; complaint themes + dev-responses stay P4 and surface as recs, not the number).
   - ✅ **Mixed dims (title / subtitle) — coarse ordinal** (`coarseOrdinalScore`): a deterministic floor (`utilisation < 20%` → 0) then snap the model's residual to the nearest of {0, 5, 10} (boundaries 2.5 / 7.5). Kills within-bucket ±1-3 drift; idempotent (snap(0/5/10) = itself), so the reuse/cache path is stable. Subtitle returns null when not observable (already excluded as `unavailable`). `description` / `icon` / `conversion` / `competitive` stay free 0-10 — acceptable because per-dimension reuse re-rolls them only when their own inputs are edited; `description` (w10) is the next quantization candidate if needed.
   - ✅ **Screenshots source = iTunes, not the crawler** (`signals.ts`). `slotsUsedOf10` uses `screenshotUrls.length` (iTunes Lookup — authoritative, structured, no LLM); `crawledScreenshotCount` is a fallback only when iTunes returns 0 (`screenshotUrls.length || crawledScreenshotCount`). An earlier `Math.max` blend overcounted — the page's `mzstatic.com/image/thumb` URLs include iPad shots, preview posters, and related-app icons, so it saturated the score at 10 for most apps. ✅ **Regression guard added** (`signals.test.ts`): two tests pin it — iTunes wins over a larger crawler count, and the crawler is the fallback only when iTunes is empty; a `Math.max` revert fails both.
   - **Honest sourcing (spec's observed/inferred/unavailable discipline).** ✅ `screenshots` and `previewVideo` are now labelled **`inferred`**, not `observed` — only slot-count / presence is observed; the quality checks need vision. Their code-scores are **Phase-A placeholders superseded at B1** (see Phase B). `ratings` stays `observed` (the average is genuinely observed; only the deferred theme/response checks aren't).
3. ✅ **Reuse, don't recompute — extended from identity (spec line 128) to scores** — done (`mastra/workflows/audit-workflow.ts`). Whole-snapshot byte-identical ⇒ reuse the stored report, **zero LLM**; otherwise a per-dimension splice reuses any dimension whose input-hash (explicit dimension→inputs map in `dimension-scorer.ts`, incl. identity version via the prompt hash) matches the prior snapshot. This is what makes the score-delta *attributable*: edit the subtitle, only Subtitle (+ keywordField, which reads it) moves.
4. ✅ **Identity at temp 0** — done (`mastra/tools/resolve-identity.ts:105`).
- **TDD (score-stability sub-bar — new in §F):** ✅ covered by `scoring/a6-score-stability.test.ts` + `dimension-scorer.test.ts` — identical re-run ⇒ byte-identical report, zero LLM; single-field edit moves only that dimension; deterministic-dimension scores are pure functions of signals. (159 hermetic tests green.)
- ✅ **Doc cleanup** — `memory/audit-memory.ts` `buildPriorContext` docstring corrected: it states it injects identity + fact-sheet only, not the ledger/change-diff (stateless generation, A3).
- ✅ **Prompt echoes the computed scores** (`scoring/prompt.ts: scoringConstraints`) — the prompt tells the model the code-computed scores and restricts title/subtitle to {0, 5, 10}, so the model's `findings` narrative matches the displayed (code-authoritative) number instead of saying "7/10" under a card that shows 5. **Both consistency follow-ups closed:** (i) ✅ previewVideo & ratings *scores* now render from `codeScore(id, signals)` (single source); (ii) ✅ the `utilisation < 20%` floor is a forced `→ 0` bullet in the same format, and subtitle's unobservable case is handled inline.
  - **Residual (small, same drift-class — optional):** the floor *threshold* `20` is still duplicated between `coarseOrdinalScore` and `scoringConstraints` (it drives a branch, so it can drift the prompt's floor decision vs. code's). Clean fix: a shared `coarseOrdinalFloor(id, signals): number | null` called by both. Cosmetic-only: the ratings nudge-note re-derives the `0.3` threshold for its annotation text (the number itself is single-sourced).
- **Beta-calibration caveats (6b golden-set retune, spec §C/§E — tracked, not blockers):** (a) *coarse-ordinal boundary instability* — a model wavering 7↔8 flips 5↔10 (a 5-pt swing on title's w20); mitigate with hysteresis or 5 anchors if it surfaces. (b) *the `utilisation < 20%` → 0 floor is harsh* — it zeroes the whole dimension on one of four checks (a short brand title like "Hulu" scores 0); consider a low-ordinal cap or lower threshold. Both are tuning of fixed beta defaults, versioned at 6b — never silent drift.

**A7 · Post-review correctness fixes (code-review batch, each verified against the code).**

**Status: ✅ all six applied, committed & verified** — `tsc` is clean except the pre-existing `routes.ts` Hono skew; 186 tests green. **Three residuals (not blockers):** (a) the orphaned-occurrences fix is **not yet pinned by a test** — add the §F "occurrences = 2 after two audits of one `rec_key`"; (b) wiring `tsc --noEmit` into the gate is **blocked by the pre-existing `routes.ts` error** — fix or exclude it first; (c) `SCORER_VERSION` is in the per-dimension hash and `rubricVersion` in the whole-snapshot guard, but `SCORER_VERSION` is **not** in the whole-snapshot guard — a `coarseOrdinalScore` snapping change (not echoed in the prompt) with unchanged `rubricVersion` would still serve a stale cached report; store + check `scorerVersion` on the snapshot for symmetry.

*Must-fix before merge:*
- **TS build is broken** — `IntentTag` is used but not imported in `audit-memory.ts:39` (`tsc` → TS2304, twice). One-line import fix (it's exported from `domain/recommendation`). **Wire `tsc --noEmit` into the test gate** — vitest strips types, so it missed this *and* a stale `audit-smoke.test.ts:51` 2-arg `produceAuditDraft` call *and* duplicate-`AppListing` type errors. "186 green" never meant "compiles."
- **Orphaned rec occurrences** — `persistAudit` calls `recordOccurrence(rec.id, …)` (`audit-memory.ts:257`) with a freshly-minted id, but `upsertRecommendation` keeps the *original* row id on `ON CONFLICT` (`libsql-storage-client.ts:106-119`, by design). Every re-raised rec then logs its occurrence under an id absent from `aso_recommendations` (no FK) → belief-accumulation undercounts (stuck at the first audit). Fix: record against the stored row's id (the dismissed-path at line 253 already does). Add a §F test: occurrences = 2 after two audits of one `rec_key`.

*Correctness:*
- **Human-confirmed `nicheBand`** — `resolveWithHistory`'s no-flip reuse (`human-confirm.ts:133`) sets `categoryBand:'high'` but **not** `nicheBand`, pairing the human-confirmed niche with the classifier's fresh band. Mirror `applyHumanDecision`: `nicheBand: niche ? 'high' : null`.
- **Reuse staleness (whole-snapshot + per-dimension)** — `listingUnchanged` (`audit-workflow.ts:243`) and `dimensionInputHash` (`dimension-scorer.ts`) key only on prompt/input hashes, not on `rubricVersion` or a scorer-code version (both already stored in the snapshot). A weight retune or a Phase-B `codeScore` upgrade with unchanged inputs silently serves stale scores. Fold a rubric/scorer version into both reuse keys (or invalidate on deploy). *(Supersedes the earlier "guard reuse on rubricVersion/modelId" note; also de-risks the B1 vision swap.)*
- **Replay drift** — `replay.ts` re-implements `assembleReport`'s normalization; extract a shared `normalize(dimensions, weightFn)` used by both (replay varies the weight column, so it can't call `assembleReport` directly).

*Observability:*
- **Swallowed classifier errors** — `parseClassificationText` (`mastra/tools/resolve-identity.ts:89`) returns `UNKNOWN` on any parse/schema failure with no log, so a broken classifier is indistinguishable from a legitimate "Unknown" (it does still escalate via the low band, so not silently-wrong). Log/flag the parse failure.

*Reviewed and rejected (do NOT action as written):*
- "Domain-flip buries the human confirmation forever" — **overstated.** It's the escalate/re-ask path (`escalate:true` persists and re-prompts). Real only if the human never re-engages after a genuine flip, which then drops the `human_confirmed` tier — minor hardening, not "forever," and `fresh.source` is `resolved`, not `lite`.
- "Contradiction guard leaves both recs proposed" — **by design** (spec §C: *flag* before emit, don't suppress); both are surfaced in `contradictions[]`. Auto-suppression would be a product change, not a bug fix.
