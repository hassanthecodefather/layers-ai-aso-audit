...
## Phase D · P4 Deep Review Analysis [live Gemini + embeddings] — 🚧 canonical path built (319 tests); `other`-bucket embedding path OPEN

**D0 · Capture review fixtures [pure] — ✅ built (`c0a81e9`).** Two frozen `Review[]` fixtures (`reviews/__fixtures__/rivian.reviews.sample1.json`, `.sample2.json` perturbed) for the §F P4 dedup tests.

**D1 · Fetch — ✅ built (`4147ab0`).** `Review` schema extended with **`id`** (from the RSS `<id>`) and **`appVersion`** (`im:version`); `fetchReviews` paginates up to **10 pages × 50 = ~500/country**; cap labelled industry-observed.
- ⚠️ **Open decision not yet resolved:** the plan required **verifying the RSS `<id>` is stable** across the 25→500 window before keying `respond_to_reviews` on it (else fall back to a content hash). The code uses the raw `<id>` directly without that verification — if the id proves unstable, review-response dedup drifts. Confirm with the 500-review fetch in hand, or switch to author+date+body hash.

**D2 · Themes + routing + A3 re-classification — 🚧 canonical ✅ built (`1777f58`/`a68671a`); `other`-bucket embedding ⬜ NOT built.**
- ✅ **Multi-instance graduation:** the `Referent` union added `theme {bucket,text}` + `reviewId`; `fix_complaint_theme`/`respond_to_reviews` are now multi-instance. `valueKeyFor` for `theme` → the **bucket id** (stable for **named** buckets). `reviews/themes.ts: analyzeThemes()` — one LLM pass, 15-bucket taxonomy, per-version sentiment delta, `taxonomy_version: theme-taxonomy@1`; wired into the workflow + prompt. Feature requests routed to human hand-off (not ledgered).
- ⬜ **`other`-bucket embedding fallback — NOT built (the §F P4 + §C gap).**
  - **No embeddings seam exists** (no `GeminiEmbeddingProvider`, no cosine). `themes.ts` only flags `isUnresolved = (bucket === 'other')`; nothing resolves it.
  - **`value_key` for an `other` theme is the literal `'other'`** (`valueKeyFor` returns `referent.bucket`). This is the **exact merge bug the spec forbids** ("⚠️ never return literal `'other'`") — two distinct novel complaints collapse to one `rec_key`/row, and a dismissed novel complaint can resurface/overwrite. **Live whenever 2+ `other` complaints arise.**
  - **§F P4 "both paths" is therefore not green.** The acceptance requires: for an `other` theme, equivalent complaints collapse via the embedding fallback (cosine ≥ 0.85) **and** distinct ones stay separate, asserted across the perturbed sample. The suite tests only `other` → `isUnresolved: true` — a weaker bar.
  - **To close:** build the **embeddings seam** (`GeminiEmbeddingProvider.embed()` via the OpenAI-compat endpoint, same key; cosine in code; pin + store the embedding model id) and a `resolveThemeKey` that gives each `other` complaint a **stable per-complaint `value_key`** via cosine ≥ 0.85 against prior `other`-theme texts; add the both-paths §F P4 test (equivalent collapse, distinct separate, across the perturbed fixture). Until then, at minimum neutralize the literal-`'other'` key so distinct novel complaints don't silently merge.
- ✅ **§F P4 canonical path green:** pagination to cap; per-version delta only above min sample; named-bucket themes keep a stable `value_key`; de-placeholder guard holds for **named** buckets (distinct named complaints → distinct rows). The `other`-path half of §F P4 remains open per above.

**D3 · Function-grounded competitors (replaces genre-based `fetchCompetitors`) — ⬜ scoped, gated on the decision-#6 egress review.**
*Why: `fetchCompetitors` pulls by `primaryGenre`, so a cross-domain app (Rivian → Travel) gets category peers (Expedia/Booking) and the gap analysis is anchored on the wrong competitors (C-FU2 was the near-term band-aid). AppKittie's `get_keyword_difficulty.topApps` returns the apps that actually rank for a keyword — the real function-matched peers — confirmed live (`"electric vehicle charging"` → PlugShare / ChargePoint / Electrify America / EVgo / Tesla, 50 ranked apps with app ids + ratings).*
- **The chain (identity-grounded):**
  1. **Seed keywords from the *resolved identity / function*, not the listing text** — this is the load-bearing insight: the identity layer knows Rivian is an EV-companion despite the Travel listing, so it seeds EV terms → `topApps` returns EV peers. Seeding from listing text alone stays polluted for a cross-domain app. (Closes the loop: identity → seed keywords → topApps → competitors.)
  2. Query AppKittie `get_keyword_difficulty` for the seed keywords → take `topApps` (filtered through `aso_competitor_tombstones`).
  3. **Fetch those competitors' listings via iTunes Lookup** (free, Apple) — **not** AppKittie `get_app_detail` — and tokenize **title + subtitle + description**.
  4. Feed into the gap analysis in place of genre peers.
- **Disciplines (spec-mandated):**
  - **Egress (decision #6 — the gate).** Keep AppKittie egress at the **keyword level only** (send seed terms, receive `topApps`); route the competitor-listing fetch through **iTunes**, never AppKittie app-id lookups. But note this makes AppKittie **load-bearing for competitor discovery**, which is exactly what #6 cautions against (*"prefer a non-competitor provider for load-bearing queries"*) — so the #6 review must explicitly decide: keep it non-load-bearing enrichment behind the swappable seam, or source discovery from a non-competitor (AppTweak/AppFigures). **This review is the gate; D3 is not built until it's done.**
  - **Honesty:** a competitor's **keyword field is still never observable** — tokenizing their title/subtitle/description gives *observed metadata*, but gap conclusions stay **`inferred`**.
  - **Tombstones:** filter `topApps` through `aso_competitor_tombstones` (app-scoped) so a human-rejected peer never re-surfaces.
  - **Cost + reuse:** adds AppKittie credits (seed queries, 10 cr each) **and** iTunes Lookups (N competitor fetches, subject to the P5 pacer/soft-ban budget). Apply a per-audit cap on seeds + competitors, and the **store-raw / `selectCandidateResult`-style reuse** so an unchanged re-audit doesn't re-query or re-spend; E1's entity cache makes it cheaper still.
- **TDD:** identity-seeded query produces function-matched peers for the Rivian fixture (EV apps, not travel); tombstoned peers excluded; competitor gap conclusions labelled `inferred`; unchanged re-audit makes zero AppKittie/iTunes competitor calls (reuse).
