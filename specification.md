...
- **The signal stack, strongest first.** The developer's identity and their other apps; the bundle id reverse-DNS (com.rivian.*); the permission & privacy labels and device capabilities (Bluetooth, location, CarPlay → a companion-to-a-device, not a travel agency); in-app-purchase names; the marketing/support URL's own page (fetched and **cited**); the review vocabulary (what users say they actually **do** — "precondition my truck", "check charge"); and the off-store footprint, cited.
  - **Where each comes from (honest sourcing).** Developer + other-apps, bundle-id, and review vocabulary come from the iTunes **Lookup** feed; **IAP names and the App-Privacy "nutrition" labels are on the product page** (via the crawler, not Lookup); **iOS permission *usage strings* are binary-only and not publicly observable** — that family is modelled but reports `not observed` until a binary source exists. So the deterministic signals that actually fire today are developer, bundle-id, marketing-domain, and reviews; IAP / privacy-label corroboration rides the existing crawler as enrichment, and the permission family stays honest-`unavailable`.
...
- On a new audit, the **reconciliation layer reads past output** (in code, not the generation prompt): it can say "you applied this, here's the new state" instead of starting from zero. The model still scores the listing fresh; memory annotates the result, it doesn't steer what gets proposed.
...
Memory must live in **persistent storage** — it has to survive restarts and deploys, or it isn't memory. In beta we use **LibSQL**, in-process: it gives real persistence with no external service to run, secure, or pay for at one-user volume. Add a small **memory** module with the tables defined in the [Build Appendix](#build-appendix). **Recommendation generation is a pure function of the current listing (plus the grounded identity) — the prior-recommendation ledger is *not* injected into the scoring prompt.** Feeding past recs into generation makes the model *diversify away* from what it already proposed, so the rec set drifts and the ledger grows every run (observed live, even at temp 0). It isn't needed anyway: the current listing already encodes what was applied, so the model won't re-propose it. History is consumed **after** generation, in code — dedup, applied-detection, contradiction-guard, belief-accumulation, and the "you applied this" report. Mastra's storage primitives support both LibSQL and Postgres *(assumption to verify against the pinned Mastra version before P6 — the whole "config change, not migration" claim rests on it)*.
...
aso_recommendations                  -- the suggestion ledger
  ...
  value_key       TEXT NOT NULL      -- code-computed from the model's TYPED referent (keyword/themeId/reviewId/country), never from free-text after/title; '' for single-instance intents (see §C)
...
**`rec_key` = stable hash of `(dimension, intent, target_field, value_key)`** — *not* the wording. `value_key` is the **normalized candidate value** that distinguishes instances of a multi-instance intent. **It is computed in code from a *typed referent* the model emits in its `AuditDraft` — never derived from the model's free-text `after`/`title`.** Prose varies run-to-run, so a prose-derived key mints a fresh row every time the model rewords the same idea (the exact failure that breaks dedup live — and that a fixed-answer LLM stub *hides*, because the stub returns identical text both runs). So the model emits `intent` (a closed enum) **plus a typed referent**, and code derives the key. The referent per multi-instance intent: **`add_keyword` / `remove_wasted_term`** → the keyword token; **`fix_complaint_theme`** → a canonical complaint-theme id (the closed taxonomy below); **`respond_to_reviews`** → the review id (Apple's stable RSS id, *extracted, not generated*); **`localise_storefront`** → the country code. **Every other intent is single-instance** — `value_key` empty, keyed on dimension+intent+target_field: `add_preview_video`, `enable_promo_text`, `rebalance_title_subtitle`, `reposition_identity`, `improve_icon_legibility`, `improve_description_hook`, `reorder_screenshots`. (All twelve `IntentTag`s are now classified; leaving one ambiguous is what let a prose-keyed duplicate slip in.) Without `value_key`, two *different* keyword suggestions for the subtitle would hash identically and the second would silently upsert over the first — the exact collision P1 exists to prevent. Two runs that produce the **same** suggestion (same `value_key`) phrased differently still map to one `rec_key`, so re-suggestion is an `upsert` (bump `last_seen_at`), not a duplicate row. The **contradiction guard** is then a pure lookup: a new rec whose `rec_key` matches a `dismissed`/opposite row is flagged before emit.
...
- **ID-lite:** given a fixture listing, resolver returns category + band + a tally citing each family to its source; Rivian fixture → category Travel-vs-vehicle **cross-domain → escalate**; TikTok/Spotify fixtures → **zero asks**; on-store-only fixture → band **≤ medium**. Identity row written to `aso_identity_versions` (stage=`lite`). **The escalate path surfaces the widened confirm step** (here's the identity + competitors → confirm / correct / pick a candidate); a human correction **persists as `source=human_confirmed` and is reused, not re-litigated**, on the next run. *(The no-engagement branch — suppress identity-rewriting recs + stamp "identity unconfirmed" — is the complement, not a substitute.)*
- **P1:** audit the same app twice → second run reads the first; a re-raised suggestion yields **no duplicate ledger row**, **including a *reworded* re-raise — same typed referent, different model prose — which must still collapse to one row** (so a key derived from generated text can't pass; the stub returns differently-worded outputs across the two runs to force the case); **yet two distinct `add_keyword` recs for the same field survive as two rows** (the `value_key` discriminator works — the test asserts *all* directions); contradiction guard fires on a reversed rec; rubric-weight replay recomputes a stored draft's score with **zero LLM calls** (assert call count = 0).
...
|---|---|---|
| ID-lite | `mastra/tools/resolve-identity.ts`, `domain/identity.ts` | `workflows/audit-workflow.ts` (resolve before score, consuming the already-gathered listing — no second iTunes fetch; no separate `identify-app.ts` change required) |
